...
def _edt_cpu_fallback(data: torch.Tensor) -> torch.Tensor:
    """CPU fallback for EDT using scipy when triton/CUDA is unavailable (e.g. MPS)."""
    from scipy.ndimage import distance_transform_edt
    import numpy as np
    device = data.device
    data_np = data.cpu().numpy().astype(bool)
    out = np.stack([distance_transform_edt(d) for d in data_np]).astype(np.float32)
...
def edt_triton(data: torch.Tensor):
    ...
    Uses the Triton CUDA kernel when available, falls back to scipy on CPU/MPS.

    Args:
        data: A tensor of shape (B, H, W) representing a batch of binary images.

    Returns:
        A tensor of the same shape containing the EDT (equivalent to batched
        cv2.distanceTransform with cv2.DIST_L2).
    """
    assert data.dim() == 3
